...
container.innerHTML += '<h1>Nombres de promos y nombre y edad de alumnas</h1>';
...
for (let i = 0; i < promos.length; i++) {
  ...
  for (let j = 0; j < promos[i].students.length; j++) {
    ...
    containerListItem.innerHTML += `<li>${promos[i].students[j].name}, ${promos[i].students[j].age}</li>`
