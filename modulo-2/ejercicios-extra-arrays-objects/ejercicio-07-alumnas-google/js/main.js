...
container.innerHTML += '<h1>Nombres de las promos y los nombres la edad y el id de las alumnas que trabajan en Google</h1>';
...
for (let i = 0; i < promos.length; i++) {
  ...
  for (let j = 0; j < promos[i].students.length; j++) {
    ...
    for (let k = 0; k < studentsWorkingInGoogle.length; k++) {
      if (studentsWorkingInGoogle[k] === promos[i].students[j].id) {
...
/*Utilizo un tercer ciclo for anidado para comparar el array de studentsWorkingInGoogle con el id de cada estudiante de cada promo. Sólo pinto los que coincidan */
